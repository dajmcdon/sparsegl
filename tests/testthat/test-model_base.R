...
test_that("largest squared singular value", {
  ...
  expect_equal(calc_gamma(x2, ix = 1, iy = 4, bn = 1, loss = "ls"), RSpectra::svds(x2, 1, 0, 0)$d^2 / 3)
  
  res <- calc_gamma(x2, ix = c(1,3), iy = c(2,4), bn = 2, loss = "ls")  # 3-by-4 matrix divided into two 3-by-2 matrices
  res1 <- double(2)
  ...
  expect_equal(calc_gamma(A, ix = 1, iy = 8, bn = 1, loss = "ls"), RSpectra::svds(A, 1, 0, 0)$d^2/6)
  
  res <- calc_gamma(A, ix = c(1,3,6), iy = c(2, 5, 8), bn = 3, loss = "ls") # 6-by-8 matrix divided into 3 matrices
  res1 <- double(3)
  ...
  expect_equal(calc_gamma(A, ix = 1, iy = 10, bn = 1, loss = "ls"), RSpectra::svds(A, 1, 0, 0)$d^2 / 8)
  
  res <- calc_gamma(A, ix = c(1,6), iy = c(5, 10), bn = 2, loss = "ls")  # 8-by-10 sparse matrix divided into two 8-by-5 matrices
  res1 <- double(2)
