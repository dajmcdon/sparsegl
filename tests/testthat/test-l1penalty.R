test_that("Test if function throws error as expected or automatically perform 
  ...
  out2 <- sparsegl(X, y0, group = gr, family = "binomial", pfl1 = rep(2, 15))
  expect_equal(out1$beta, out2$beta)
...
test_that("function behaviors changed by pfl1", {
  ...
  out2 <- sparsegl(X, y, group = gr, pfl1 = pfl1_2, asparse = 1)
  ...
  out2 <- sparsegl(X, y, group = gr, pfl1 = pfl1_2, lambda = out1$lambda, asparse = 1)
  ...
  out <- sparsegl(X, y, group = gr, pfl1 = seq(1, 60, by = 10), asparse = 1)
