...
# Example usage
This section provides a simple guidance for how to employ this package for fitting the regularization paths for sparse group-lasso penalized learning problems. For simplicity, we only show the case when using linear regression model. We first import the sparsegl package, and start with a simulated regression dataset including an input matrix of predictors $\mX$, a real-valued response vector $\vY$, and a vector indicating the grouping structure:
```{r}
library(sparsegl)
```
...
>>>>>>> 3d6607c1072e7ef265b7ccce5c3cb61c750bb0ea
...
simulate <- function(change_list,
  ...
  loss <- match.arg(loss, c("ls", "logit"))
  df <- list()
  k <- 1
    ...
    for (j in seq(50)) {
      ...
      if (loss == "ls") {
        ...
        df[[k]] <- as.data.frame(summary(
          microbenchmark(sparsegl = sparsegl(X, y, groups, loss = loss, asparse = 0),
                         SGL = SGL(data, groups, type = ifelse(loss == "ls", "linear", loss), alpha = 0, nlam = 100),
                         gglasso = gglasso(X, y_gglasso, groups, loss = loss),
                         times = 1))) 
        } else {
          df[[k]] <- as.data.frame(summary(
            microbenchmark(sparsegl = sparsegl(X, y, groups, loss = loss, asparse = 0.5),
                           SGL = SGL(data, groups, type = ifelse(loss == "ls", "linear", loss), alpha = 0.5, nlam = 100), 
            microbenchmark(sparsegl = sparsegl(X, y, groups, loss = loss),
                           SGL = SGL(data, groups, type = ifelse(loss == "ls", "linear", loss)), 
                           times = 1))))
        }
      df[[k]] <- df[[k]] %>% 
        select(expr, mean) %>% 
...
plot_box <- function(df, loss = c("ls", "logit")) {
  loss <-
  plot <- ggplot(df, aes(x = change_list, y = mean, fill = expr)) +
    geom_boxplot(fatten = 0.5) +
    theme_bw() + 
    theme(legend.position = "bottom", legend.title = element_blank()) +
    xlab("Change values") +
    ylab(paste("Running Time (ms) for", 
               switch(loss,
                      ls = "Linear Regression Model",
                      logit = "Logistic Regression Model"))) +
    facet_grid(vars(within_group), vars(change_feature), 
...
within_p <- simulate(change_list = c(20, 60, 100, 200, 500), change_feature = "p", within_group = TRUE, group_size = 5, loss = "ls")
within_n <- simulate(change_list = c(20, 60, 100, 200, 500), change_feature = "n", within_group = TRUE, group_size = 5, loss = "ls")
without_p <- simulate(change_list = c(20, 60, 100, 200, 500), change_feature = "p", within_group = FALSE, group_size = 5, loss = "ls")
without_n <- simulate(change_list = c(20, 60, 100, 200, 500), change_feature = "n", within_group = FALSE, group_size = 5, loss = "ls")
```


```{r, simulation log, include = FALSE}
log_within_p <- simulate(change_list = c(20, 60, 100, 200, 500), change_feature = "p", within_group = TRUE, group_size = 5, loss = "logit")
log_within_n <- simulate(change_list = c(20, 60, 100, 200, 500), change_feature = "n", within_group = TRUE, group_size = 5, loss = "logit")
log_without_p <- simulate(change_list = c(20, 60, 100, 200, 500), change_feature = "p", within_group = FALSE, group_size = 5, loss = "logit")
log_without_n <- simulate(change_list = c(20, 60, 100, 200, 500), change_feature = "n", within_group = FALSE, group_size = 5, loss = "logit")
```

```{r, concat, include = FALSE}
df1 <- bind_rows(within_p, within_n, without_p, without_n)
df2 <- bind_rows(log_within_p, log_within_n, log_without_p, log_without_n)
```


```{r, echo = FALSE, eval = FALSE, fig.cap = "Running Time Comparison", fit.width = 5, fig.height = 5}
plot_box(df1, loss = "ls")
plot_box(df2, loss = "logit")
```

\begin{figure}[htbp]
  \includegraphics{fig/unnamed-chunk-6-1.pdf}
  \includegraphics{fig/unnamed-chunk-6-2.pdf}
  \caption{Running Time Comparison for Linear Regression / Logistic Regression}
\end{figure}
