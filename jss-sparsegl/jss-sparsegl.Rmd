...
For each combination, the elapsed time (in milliseconds) is recorded for fitting regularization path at 100 $\lambda$s' values by constructing models using \pkg{sparsegl}, \pkg{SGL} and \pkg{gglasso} in terms of the existence of within-group sparsity over 100 independent datasets. The boxplots illustrate the consumed time on each run.
...
df <- readRDS("./running-time-records-current")
plot_box(df)
