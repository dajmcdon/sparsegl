...
\item Allows for interval constraints on the coefficients and differential weights on the coefficients;
...
```{r dwi-fit, echo=FALSE, eval=TRUE, fig.cap="The group norm of the 61 groups based on neuroanatomical structure against the magnitude of the group penalty. The fit was produced using \\texttt{sparsegl()} to estimate the group lasso ($\\alpha=0$)."}
...
```{r dwi-model-description, out.width="85%", fig.cap="Depiction of the model components, showing how the streamlines and voxels are set. Each voxel covers N rows, corresponding to the number of angles measured. Related to the rows covered by a voxel, the streamlines that crosses the voxel have a value, representing the theoretical signal yielded by a streamline. Otherwise, if the streamline does not cross the voxel, the respective rows have value of zero."}
include_graphics("fig/model-description.pdf")

\citep{pestilli2014} formulated an optimization model that takes as input a set of brain connections generated using tractography algorithms and predicts the MRI diffusion signal via optimization (\cite{pestilli2014, daducci2015}).
The model of Pestilli had no regularization during the optimization. \cite{aminmansour2019} extended the problem to include a group-regularization in the model optimization (this is an approach recently followed up by Schiavi et al.)
In this study, we implemented the \cite{aminmansour2019} problem, using the sparsegl we show results of feasibility and efficiency of the modeling using the toolbox.

The model performs DWI signal prediction by using the tractogram as predictors, simulating for each streamline what would be its signal given the voxels they are located at and the applied gradient fields, shown in \autoref{fig:dwi-model-description}.
The software optimizes the model using sparse group lasso, allowing custom parametrization for the rate of regularization applied to each group and to each predictor.
For this specific domain, we penalize groups proportionally by their cardinality and streamlines by their equivalent physical length, in brain space.
\autoref{fig:dwi-model} depicts a reduced version of the model, for illustration purposes.
For our study, we used one subject from the Human Connectome Project \cite{vanessen2012}, which contains datasets used for methods benchmarking.
The full brain tractogram has 3M streamlines, and only the streamlines identified as being part of the Arcuate Fasciculus were selected.
In comparison, \citep{aminmansour2019} propose an algorithm based on Orthogonal Matching Pursuit to estimate a group-regularized linear model.
The data used in the study measures diffusion in 11,823 voxels using 96 magnetic field angles and attempts to reconstruct the white matter using the ENCODE method \cite{encode2017}, resulting in 1057 orientations and 868 fascicles.
Converting this into a linear regression problem (rather than a tensor regression problem) results in a linear model with $n\approx1M$ and $p = 868$.
In comparison, 77,630 voxels measured on 90 angles, with a tractography of 10,244 streamlines. The resulting linear model has $n\approx6.9M$ and $p\approx88K$, about $72\times$ the size of the previous analysis.
The design matrix would occupy nearly 533 GB if it was dense, but since it is only about 0.0198% non-sparse, it requires 1.2 GB of memory when stored in CSC format.

Estimating the sparse group lasso using sparsegl with 12 groups based on distance and 100 different regularization rates ($\lambda$) took 1:10 minutes on a PC with Intel i7-11700K and 64 GB of RAM, with a peak of 6.15 GB of memory usage.
\autoref{fig:dwi-fit} shows the group norm of the 12 groups against the magnitude of the group penalty.
...
Daniel J. McDonald was partially supported by the National Science Foundation (DMS–1753171) and the National Sciences and Engineering Research Council of Canada (RGPIN-2021-02618). The analysis in \autoref{sec:trust} is based on survey results from Carnegie Mellon University’s Delphi Group.
