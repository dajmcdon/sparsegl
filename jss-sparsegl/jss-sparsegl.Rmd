...
knitr::write_bib(c("base", "devtools", "knitr", "testthat", "usethis",
                   "rticles", "sparsegl", "glmnet", "dplyr", "magrittr",
...
trust_experts <- mutate(trust_experts, across(
  where(is.factor), 
  ~ set_attr(.x, "contrasts", contr.sum(nlevels(.x), contrasts = FALSE))
))
...
from \autoref{eq:linmod} with the variance of the noise $\epsilon$
calibrated to produce expected signal-to-noise ratios (SNR) of $\{0.1,\ 1,\ 10\}$.
We note that this design produces group sparsity, even groups have no
effect on the response, but no within-group sparsity. \autoref{fig:accuracy}
shows the results for these 9 conditions across 20 values of $\lambda$. The
$\lambda$ range is determined automatically by \pkg{sparsegl} and reused for 
the other packages. Note that \pkg{gglasso} is optimizing a different objective
function than \pkg{CVXR} or \pkg{sparsegl}. Despite this discrepancy, the objective values for \pkg{gglasso} and \pkg{sparsegl} are generally quite close,
with \pkg{gglasso} tending to be slightly higher. On the other hand \pkg{CVXR}
...
instead produce estimates near, but not exactly, 0. When $p=n=100$, \pkg{CVXR}
is actually slightly more accurate. Inspecting the estimated $\widehat{\beta}$,
it seems to have slightly less bias on the non-zero groups, producing estimates
with slightly larger magnitude.

```{r accuracy, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE, fig.cap="The \\% change in objective function for \\pkg{gglasso} and \\pkg{CVXR} relative to \\pkg{sparsegl}.", fig.height=4, fig.width=6}
...
results |>
  ...
  facet_grid(`p/n` ~ SNR, labeller = lb, scales = "free") +
  geom_line() +
  geom_hline(yintercept = 0) +
  theme_bw() +
  scale_colour_manual(values = c("darkblue", "orange"), name = "") +
  theme(legend.position = "bottom") +
  scale_x_log10(name = bquote(log(lambda)), 
