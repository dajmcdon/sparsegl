...
In the environment Python, there are two related packages implemented for solving sparse group lasso problems. \pkg{asgl} \citep{civieta2020adaptive} proposes an adaptive sparse group-lasso, which improves the variable selection accuracy by flexibly adjusting the weights in the penalization on the groups of features in terms of the feature importance. Another breakthrough of this package is that the loss function being penalized is in the framework of quantile regression, inducing two dimensionality-reduction techniques, Principal Component Analysis and Partial Least Squares for tackling high-dimensional optimization problems. However, one drawback of this package is that it could be time-consuming to use grid-search for hyper-parameters tuning process. Another package \pkg{groupyr} \citep{richie-halford-groupyr} is developed, which mainly depends on the scikit-optimize library implementation when tuning hyperparameters for cross-validation. It either uses an exhaustive grid search with a warm-start or the sequential model-based optimization (SMBO), while SMBO is expected to outperform grid search as it requires fewer evaluations to obtain the optimal hyperparameters. 
<!-- The third package is called \pkg{group-lasso}, which incorporates the FISTA optimizer with a gradient-based adaptive restarting scheme. In the discussion portion, the authors point out that the FISTA algorithm is not stable in the mini-batch case, which results in inefficiency when performing on a high-dimensional dataset. In the meanwhile, the authors argue that the package could still recover the sparsity patterns of the data in his experiments. -->
...
```{r}
fit <- sparsegl(X, y, group = groups, loss = "ls")
```
...
```{r, results = "hide", fig.width = 4, fig.height = 2}
fit_logit <- sparsegl(X, y1, group = groups, loss = "logit")
...
cv_fit_logit <- cv.sparsegl(X, y1, groups, loss = "logit", pred.loss = "misclass")
