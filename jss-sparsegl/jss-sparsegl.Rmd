...
library(ggplot2)
data.frame(Lambda = fit$lambda, AIC = er) %>%
  ggplot(aes(Lambda, AIC)) +
  geom_point(color = "darkblue") +
  theme_bw() +
  geom_vline(xintercept = fit$lambda[which.min(er)], linetype="dashed") +
  scale_x_log10()
```

In this simulation, the $\lambda$ that minimizes AIC is `r round(fit$lambda[which.min(er)],3)` while the CV minimizer is `r round(cv_fit$lambda.min,3)`.
