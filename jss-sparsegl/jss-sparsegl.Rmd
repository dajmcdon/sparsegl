...
The sequential strong rule \citep{tibshirani2012strong} begins from Equation \eqref{eq:subgrad-again} and makes use of the fact that we are solving for a sequence $\{\lambda_1 \geq \lambda_2 \geq \dots \geq\lambda_S\}$ of lambda parameters, rather than a single value. At each $\lambda_s$, we rely on the fact that we have solved the problem for the previous $\lambda_{s-1}$, and use this information to quickly discard many groups of predictors. Without loss of generality, for the rest of this section assume that the problem has already been solved for $\lambda_{s-1}$.
...
\end{equation}
This is essentially saying that $c_g$ is not only continuous but does not vary too much in $\lambda$; this is equivalent to saying that the function is differentiable almost everywhere and has bounded derivative. This Lipschitz assumption appears a unintuitive, and in fact, is not always true. But it turns out to be useful. 

If we knew that $\snorm{c_g(\lambda_s)}_2 < (1-\alpha)\lambda_s$ then we could safely ignore it. But we already have the solution at $\lambda_{s-1}$. By the triangle inequality,
$$
\snorm{c_g(\lambda_s)}_2 \leq \snorm{c_g(\lambda_s) - c_g(\lambda_{s-1})}_2 +
\snorm{c_g(\lambda_{s-1})}_2 \leq (1-\alpha)(\lambda_{s-1} - \lambda_s) + \snorm{c_g(\lambda_{s-1})}_2.
$$
To continue this upper bound, we want
$(1-\alpha)(\lambda_{s-1} - \lambda_s) + \snorm{c_g(\lambda_{s-1})}_2 \leq (1-\alpha)\lambda_s$ which holds if
\begin{equation}
\label{eq:strong}
\snorm{c_g(\lambda_{s-1})}_2 \leq (1-\alpha)(2\lambda_s - \lambda_{s-1}).
\end{equation}
This is the sequential strong rule for the sparse group lasso. For more details in related settings, see \citep{tibshirani2012strong}.
If Equation \eqref{eq:strong}
holds, then we discard group $g$ from the problem, before solving for $\lambda_s$. That is to say, when we move from $\lambda_{s-1}$ to $\lambda_s$, we first check this condition on all groups, using the previously computed solution for $\hat{\vbeta}(\lambda_{s-1})$, and then perform blockwise coordinate descent, using only those groups that failed this inequality.
...
Violators of the KKT condition should be put in 'active' status and the algorithm re-run with those groups included. The details of this are in the next section.
...
\begin{algorithm}[tb!]
  ...
  \label{alg:djm}
  \begin{algorithmic}[1]
  \STATE {\bfseries Input:} $\mX$, $\vY$, $\mathcal{G}$, (required) $\{\lambda_1,\ldots,\lambda_K\}$ (optional)
  \STATE {\bfseries Initialize:} $\mathcal{A} = \mathcal{S} = \varnothing$
  \FOR{$k=1$ \TO  $K$} 
  \STATE \textbf{for each} $g\in\mathcal{S}^c$, apply the strong rule add violators to $\mathcal{S}$ \COMMENT{Equation \eqref{eq:strong}}
  \WHILE{$\max_{g\in\mathcal{A}} \snorm{\hat{\vbeta}^{(g)} - U(\hat{\vbeta}^{(g)})}_2 >\epsilon$}
    \STATE For each $g\in\mathcal{A}$, update $\hat{\vbeta}^{(g)}\leftarrow U(\hat{\vbeta}^{(g)})$ \COMMENT{Equation \eqref{eq:updateStep}}
  \ENDWHILE
  \STATE \textbf{for} $g\in\mathcal{S}$, If $\snorm{S(\nabla \ell(\hat\vbeta_g),\ \lambda_k\alpha)}_2 > (1-\alpha)\lambda_k$, add $g$ to $\mathcal{A}$ 
  \STATE If there were any violations, return to Line 5.
  \STATE \textbf{for} $g\in\mathcal{A}^c$, If $\snorm{S(\nabla \ell(\hat\vbeta_g),\ \lambda_k\alpha)}_2 > (1-\alpha)\lambda_k$, add $g$ to $\mathcal{A}$
  \STATE If there were any violations, return to Line 5.
...
\citet{vaiter2012degrees} gives an explicit formula for the group lasso (without within-group sparsity), but only minor modifications are required for the sparse group lasso. We give a simplified version of their result here
