    ...
    for (j in seq(50)) {
      ...
      if (loss == "ls") {
        ...
        df[[k]] <- as.data.frame(summary(
          microbenchmark(sparsegl = sparsegl(X, y, groups, loss = loss, asparse = 0, standardize = FALSE),
                         SGL = SGL(data, groups, type = ifelse(loss == "ls", "linear", loss), alpha = 0, nlam = 100, standardize = FALSE),
                         gglasso = gglasso(X, y_gglasso, groups, loss = loss),
                         times = 1))) 
        } else {
          df[[k]] <- as.data.frame(summary(
            microbenchmark(sparsegl = sparsegl(X, y, groups, loss = loss, asparse = 0.5, standardize = FALSE),
                           SGL = SGL(data, groups, type = ifelse(loss == "ls", "linear", loss), alpha = 0.5, nlam = 100, standardize = FALSE), 
                           times = 1)))
        }
      df[[k]] <- df[[k]] %>% 
        select(expr, mean) %>% 
...
plot_box <- function(df, loss = c("ls", "logit")) {
  loss <- match.arg(loss)
  df <- df %>%
    group_by(expr, change_feature, change_list, within_group) %>% 
...
```{r, echo = FALSE, eval = FALSE, warning = FALSE, fig.cap = "Running Time Comparison", fit.width = 5, fig.height = 5}
