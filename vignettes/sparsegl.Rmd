...
* pf: a penalty vector of the length of the total number of groups bn, whose default 
value is $\sqrt{\text{bs}}$ in $(*)$ above.

* asparse: changes the weight of lasso penalty, referring to $\alpha$ in $(*)$ above: `asparse` = $1$
gives the lasso penalty only. `asparse` = $0$ gives the group lasso penalty only. 
The default value of `asparse` is $0.05$.

* lower_bnd: lower bound for coefficient values, a vector in length of 1 
or the number of groups including non-positive numbers only. Default is -$\infty$.

* upper_bnd: upper bound for coefficient values, a vector in length of 1 
or the number of groups including non-negative numbers only. Default is $\infty$.
...
under function `risk_estimate()` below.
...
* `grouped`: `grouped` is TRUE if the user designed a specific grouping of features 
in the `sparsegl()`, otherwise FALSE by default.

* "y_axis": can be set with either "coef" or "group_norm".

* "x_axis": can be set with either "log_lambda" or "norm".

If `grouped` is set with FALSE by default, `y_axis` can only be set with "coef" 
such that this function will plot at the feature level only.Each curve with a 
distinct color represents a feature. Notice that the number of curves shown on 
the plots could be less than the actual number of features since only the features
having nonzero coefficients at least at one lambda in the sequence `lambda` are 
included:

* The plot with `x_axis` = "log_lambda" is the coefficients of features against 
logarithm of `lambda` sequence (log-`lambda`). 

* The plot with `x_axis` = "norm" is the coefficients of features against a vector 
of scaling norm at each `lambda`. Each element in this vector is defined by:
$$
...
plot(fit1, grouped = FALSE, x_axis = "log_lambda")
```

If `group` is set with TRUE, this function can plot at both feature and group levels.
Again, only the features having nonzero coefficients at least at one lambda value 
in the sequence `lambda`, and their corresponding groups are considered: 

* The plot with `y_axis` = "group_norm" is the group norms against the log-`lambda` 
or scaling norm vector. Each group norm is defined by:
$$
...
the log-`lambda` or scaling norm vector. Curves are plotted in the same color if the corresponding
features are in the same group. 

```{r, message = FALSE, warning = FALSE, fig.width = 8, fig.height = 4}
plot(fit1, grouped = TRUE, y_axis = "group_norm", x_axis = "log_lambda")
plot(fit1, grouped = TRUE, y_axis = "coef", x_axis = "norm")
```

### `coef()`, `predict()` and `print()` for `sparsegl` object
...
arguments `X`, `y`, `group`, which are specified in `sparsegl()`, and also an additional
...
### `plot()`, `coef()` and `predict()` for object `cv.sparsegl`
...
* `coef()` and `predict()` work similarly to the functions `coef.sparsegl()` and 
...
at each penalty parameter `lambda` is accessible using `object$df`. Default is FALSE and 
...
coefficients of the model, which is an unbiased estimator for the exact degree-of-freedom.
Notice that it would take some time to calculate the exact degreeo-of-freedom if 
`X` is complicated. For more details about how to realize this calculation, our 
method is implemented based on the paper https://arxiv.org/pdf/1212.6478.pdf. 
