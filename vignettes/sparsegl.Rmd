...
such that this function will plot at the feature level only, and `y_axis` is not 
necessarily explicitly specified in this case. Each curve with a distinct color 
represents a feature. Notice that the number of curves shown on the plots could 
be less than the actual number of features since only the features having nonzero
coefficients at least at one lambda in the sequence `lambda` are included:
