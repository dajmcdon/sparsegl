...
* Compiled Fortran code significantly speeds up the sparse group-lasso model building process.

* So-called "strong rules" are implemented during group wise coordinate descent steps to screen out groups which are likely to be 0 at the solution.

* The design matrix `X` can be a sparse.
...
* Additional glm families may be fit (though this is typically slower).
...
* Logistic regression: 
$$
\min_{\beta\in\mathbb{R}^p}\left(\frac{1}{2n}\sum_{i=1}^n \log\left(1 + \exp\left(-y_ix_i^\top\beta\right)\right) + (1-\alpha)\lambda\sum_g \sqrt{\text{bs}}\rVert\beta^{(g)}\rVert_2 + \alpha\lambda\rVert\beta\rVert_1 \right) \qquad (**).
$$
...
use for cv error. This will return a `cv.sparsegl` object.
