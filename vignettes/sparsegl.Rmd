...
To elaborate on `x_axis` and `y_axis`:

* The plot with `x_axis` = "lambda" is the coefficients of features against 
logarithm of `lambda` sequence (log-`lambda`). 

* The plot with `x_axis` = "penalty" is the coefficients of features against a vector 
...
the log-`lambda` or scaling norm vector. Each curve with a distinct color 
